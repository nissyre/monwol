            ...
            <div class="nav-links">
                <div class="row">
                    <!-- Get Next Post -->
                        ...
                        <?php _e(' NEXT', 'nisarg'); ?></span><br><?php if (get_the_title($next_post->ID) != '') {
                    ...
                    if (!empty($prev_post)) {
                        ...
                        <div class="col-md-6 prev-post">
                            <a class="" href="<?php echo esc_url(get_permalink($prev_post->ID)); ?>"><span class="next-prev-text"><?php _e('PREVIOUS ', 'nisarg'); ?><i class="fa fa-angle-right"></i>
                                </span><br><?php if (get_the_title($prev_post->ID) != '') {
                echo get_the_title($prev_post->ID);
            } else {
                _e('Previous Post', 'nisarg');
            } ?></a>
                        </div>
        <?php
        } else {
...
if (!function_exists('nisarg_posted_on')) :
    ...
    function nisarg_posted_on() {
        ...
        $entry_meta = sprintf($entry_meta, esc_url(get_permalink()), esc_attr(get_the_time()), esc_attr(get_the_date('c')), esc_html(get_the_date()), esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_attr(sprintf($viewbyauthor_text, get_the_author())), esc_html(get_the_author()));
